...
Since the top-most extension is exponention, we apply Theorem 6.1, and
...
We have two places at which $S$ and $T$ both have first-order poles, so we need to consider the ``$b$'' coefficient,
which is a problem, because on an algebraic curve only the residue of a differential is well defined, so let's
rewrite equation 6.8:
...
These calculations are with respect to a local uniformizing variable, so the order of the pole
always drops by one under derivation.

\begin{verbatim}
v(S dx) = 0,  v(T dx) = -2 implies v(R) = -1 so that v(dR) = -2
v(S dx) = -6, v(T dx) = -7 at oo imples v(R) = -1 so that v(R S dx) = -7
\end{verbatim}

so our solution, if it exists, has two first-order poles at $(0,1)$ and $\infty$.
