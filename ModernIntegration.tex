...
\usepackage[usefamily=maxima,keeptemps=all,autoprint=false]{pythontex}
...
% Python code wraps a maximacodeblock around a series
% of calls to \maximainputlabel, \maximaoutputlabel,
% and \maximaoutput
...
% I want my maxima code to be nicely typeset using the macros above,
% so I redefine maximablock (the environment to both typeset and
% execute) to call maximacode (the environment to only execute) and
% then print the output using the above macros, instead of the
% default behavior.

\renewenvironment{maximablock}{\maximacode}{\endmaximacode\printpythontex}
