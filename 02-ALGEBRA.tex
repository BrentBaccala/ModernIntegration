...
Put another way, $u$ is a unit if there exists $u'$ such that $uu'=1$.
...
More succinctly, we can use the Maxima function {\tt resultant}:
...
factorization_problem(p) := block(
   ?princ(concat("\\maximaoutputmath{$", tex1(expand(p)), "$}")),
   ?princ("\\maximaoutputmath{\\newline}"),
   ?princ(concat("\\maximaoutputmath{{\\tt ", string(expand(p)), "}}")),
   ?princ("\\maximaoutputmath{\\newline}"),
   ?princ(concat("\\maximaoutputmath{Ans: $", tex1(factor(p)), "$}"))
)$
\end{maximacode}
