...
It is possible to extend a field (any field?) so that all polynomials
can be completely factored into linear terms.  Such a field is said to
be {\it algebraically closed}.  The {\b Fundamental Theorem of
...
behavior of bounded entire functions.
...
particularly that O2 implies that $1+1$ is always positive, never
...
Squares must be positive, so a maximal real field can be ordered in
only one way.
...
We establish some key properties of a maximal real field, starting
with the fact that all positive numbers are squares (we're introducing
order here).  Intuitively, this is because all positive real numbers
have real roots, but we do not specialize to the real line, instead
proofing the result for real fields in general.
...
The $\gamma$ term must vanish, because otherwise we could solve
for $\gamma$ as follows:

$$\gamma = \frac{-1 - \sum a_i^2 - \sum b_i^2 p}{2 \sum a_i b_i}$$

would would place $\gamma$ in the original field, an impossibility,
so the denominator must be 0, and the original expression
simplies to:
...
squares, because otherwise $-1$ could be.  Since we've only assumed
that $p$ is not a square, this implies that any sum of squares must
itself be a square.  Futhermore, since squares are always positive
(need proof), the expression on the right is strictly negative,
contradicting the assumption that $p$ is positive.  On the other hand,
if $p$ were negative, $-p$ would be expressible as a square since both
the numerator and denominator are sums of squares:

$$-p = \frac{1 + \sum a_i^2}{\sum b_i^2}$$

which implies that any sum of squares is a square.  Solving for $p$,
we see that its numerator and denominator are both sums of squares, so
$-p$ must itself be square.  Every element is either a square, or the
negative of a square, and these cases are exclusive.  This also
establishes that adjoining $i$ creates a field where every quadratic
equation is solvable using the quadratic formula.
...
By adjoining a root $\alpha$ and getting a non-real field, we arrive
at a contradiction.
...
\definition
Given an algebraic extension $E$ of a field $F$, the {\it trace} of an
element $x$ in $E$ (written ${\rm Tr}(x)$) is the sum of the element
and all of its conjugates.
\enddefinition

\theorem\label{trace in underlying field}

${\rm Tr}(x) \in F$ {\rm [Wikipedia article ``Newton's identities'']}

\proof

Write $x$'s minimal polynomial in the form:
$$\prod_{i=1}^n \left( t - x_i \right) = \sum_{k=0}^n (-1)^{k} a_k t^{n-k}$$
and note that the $n-1^{\rm th}$ coefficient in the polynomial is
${\rm Tr}(x)$, and that all of the coefficients are in $F$.
