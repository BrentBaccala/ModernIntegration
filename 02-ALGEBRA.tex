...
Both rings and fields are defined over sets with two operators,
...
(read: there exists) to abbreviate this table.  These symbols
will rarely be used again.}

\begin{center}
\begin{tabular}{l l l r}
   associative law of addition	& $\forall a,b,c \in {\cal R},$ & $(a+b)+c = a+(b+c)$ &(R1)\cr
   associative law of multiplication & $\forall a,b \in {\cal R},$ & $(ab)c = a(bc)$ &(R2)\cr
   commutative law of addition	& $\forall a,b \in {\cal R},$ & $a+b = b+a$ &(R3)\cr
   distributive law   & $\forall a,b \in {\cal R},$ & $a(b+c) = ab + bc$ &(R4)\cr
   existence of an additive identity  & $\exists 0 \in {\cal R}, \forall a \in {\cal R}$, & $0 + a = a$ &(R5)\cr
   invertibility of addition & $\forall a \in {\cal R}, \exists b \in {\cal R}$, & $a + b = 0$ &(R6)\cr
\end{tabular}
\end{center}


A {\it field} ${\cal F}$ obeys all the ring axioms (thus all fields are also
rings), as well as the following:

\begin{center}
\begin{tabular}{l l l r}
   existence of a multiplicative identity & $\exists 1 \in {\cal F}, \forall a \in {\cal F},$ & $1a = a$ &(F1)\cr
   invertibility of multiplication & $\forall a \ne 0 \in {\cal F}, \exists b \in {\cal F},$ & $ab = 1$ &(F2)\cr
\end{tabular}
\end{center}

Notice that I didn't list the commutative law of multiplication.  This
is because a substantial theory has been developed around {\it
non-commutative} rings and fields, probably because matrix
multiplication (a critically important example) is non-commutative.  A
commutative ring is said to be {\it abelian}, and most authors require
a field to be commutative; reserving the term {\it skew field}
to refer to a (possibly) non-commutative field.  Since all of our
rings and fields will be commutative, I will omit this additional
terminology and adopt a further ring axiom:

\begin{center}
\begin{tabular}{l l l r}
   commutative law of multiplication & $\forall a,b \in {\cal F},$ & $ab = ba$ &(R7)\cr
\end{tabular}
\end{center}
...
defined due to the invertibility of addition.  Division, however, is
not.  Since the ring axioms do not require multiplication to be
invertible, there is no guarantee that we can carry out division
in a ring.  A field, on the other hand, is a mathematical system
in which all four elementary operations --- addition, subtraction,
...
Which brings us to when I excluded zero as a
possible denominator.  We do this because otherwise our operations
aren't cleanly defined on these equivalence classes. ${1\over0}$
is not equivalent to ${0\over1}$ (since $1\cdot1\ne0\cdot0$),
so ${1\over0}$ must have a multiplicative inverse (by axiom F2); i.e,
some fraction ${a\over b}$ must exist which when multiplied
by ${1\over0}$ produces ${1\over1}$, yet by Chapter 1's
zero theorem, no such element $b$ can exist so that $1b=0$.
Excluding zero as a possible denominator ensures that our
...
${\cal F}[x]$ is a ring, but except for an important special case (an
algebraic variable), not generally a field.  It is, however, an
...
field whatsoever.  Given a polynomial ring ${\cal F}[x]$, we can use
the field operations provided by ${\cal F}$ to divide any two elements
from ${\cal F}[x]$ using polynomial long division and obtain a
remainder and a quotient.
...
+ r$ (dividend = quotient times divisor plus remainder), can be
rearranged to read $r = D - qd$, which shows that any common divisor
of the dividend and the quotient can be divided out from the right
hand side of the equation, so must also divide the left hand side.
Thus, common divisors of the dividend and divisor are preserved
in the remainder.
...
so that $0x=c$ or $0x=d$ or $x0=c$ or $x0=d$, which by the zero
theorem implies that either $c=0$ or $d=0$.  This proves I1.  Thus, a
unique factorization domain is also an integral domain.  Also, ${\cal
F}[x]$ is a unique factorization domain unless $x$ is algebraic (read
on for details).
...
exist since the square of $i$ is -1; $i$ is thus {\it algebraic} and
this makes all the difference.  The number 9 can be factored two
different ways in this ring: $9=3\cdot3=(4-i)(4+i)$.  It's not too
hard to see that 3 can't be multiplied by any Gaussian integer to form
either $(4-i)$ or $(4+i)$, so U1 is not satisfied.  The Gaussian
integers form an integral domain, but not a unique factorization
domain.
...
Compute the partial fractions expansion of $${x^2 + 3x + 2}\over{(x-2)^2(x+1)}$$

The degree of the numerator is 2 and the degree of the denominator is 3,
so the fraction is already proper.  We begin by solving the polynomial
...
surprising since an initial assumption is that the denominator
factors have no common factor.
...
$${{x^2 + 3x + 2}\over{(x-2)^2(x+1)}} = {4\over{(x-2)^2}} + {1\over{(x-2)}}$$
