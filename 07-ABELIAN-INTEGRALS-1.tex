...
puiseux4(f,x,y,x0,y0,deg,rf) :=
 block([result: [], lastslope : 0, sign, mp, pairs],
...
matrix_for_basis_at_x0(f,x,y,divisor,basis,x0) :=
 block([ys, cycles, A, required_degrees, pairs, expansions], local(A),
    ...
    cycles: map(lambda([y0], block([expansion: puiseux(f,x,y,x0,y0,0,1)],
       if length(expansion) # 1 then
          error("riemannroch can't handle divisors at singularities"),
       hipow(lhs(expansion[1][2]), t))), ys),
    ...
    /* XXX should only need to call puiseux once */

    A[i,j] := coeff(puiseux(f,x,y,x0,pairs[i][1], pairs[i][2],basis[j])[1][1], t, pairs[i][2]),
    ev(genmatrix(A,length(pairs),length(basis)), puiseux_fracexp: false)
)$
 ...
 block([basis,xs],
    ...
    for x0 in xs do block(
      for i: 1 thru length(basis) do
        basis[i] : basis[i] * x^(-lopow(puiseux(f,x,y,x0,false,0,basis[i])[1][1], t))
    ),
...
normal_matrix_at_inf(f,x,y,basis) := block([expansions, cycles, indices, signs, colord, A], local(A),
  ...
  /* XXX this signs trick only works for cycles with 1 or 2 sheets */
...
normal_vector_at_inf(f,x,y,func) :=
 block([expansion, col_ord],

    /* [[expansion, t^n=1/x],...] */
    expansion : puiseux(f,x,y,inf,false,5,func),

    /* column order */
    col_ord : lmin(map(lambda([pe], lopow(pe[1], t)), expansion)),

    flatten(map(lambda([pe], block(
      sheets : hipow(lhs(pe[2]), t),
      makelist(coeff(pe[1], t, col_ord + i),i,0,sheets-1)
    )), expansion))
)$

ormal_matrix_at_inf(f,x,y,basis) :=
  transpose(apply('matrix, map(lambda([func], normal_vector_at_inf(f,x,y,func)), basis)))
$
...
riemannroch2(f,x,y,divisor) := block([finite_divisor, infinite_divisor, basis, colord, a],

  finite_divisor : sublist(divisor, lambda([d], first(first(d)) # inf)),
  infinite_divisor : sublist(divisor, lambda([d], first(first(d)) = inf)),

  basis : basis_of_divisor_normal_at_inf(f,x,y,finite_divisor),
...
riemannroch(f,x,y,divisor) := block([finite_divisor, infinite_divisor, basis, colord],
  ...
  if length(infinite_divisor) # 0
     ...
     infinite_divisor : map(lambda([d], [[a, d[1][2]], d[2]]), infinite_divisor),
     ...
     map(lambda([f], ratsimp(subst([x=1/(x-a)], f))), basis)

  ) else riemannroch2(f,x,y,divisor)
)$
...
puiseux(f,x,y,inf,false,5,(3*x+2*y)/(x-2));
puiseux(f,x,y,2,3,5,(3*x+2*y)/(x-2));
puiseux(f,x,y,2,3,5,(y+3)/(x-2));
puiseux(f,x,y,2,3,5,y);

normal_matrix_at_inf(f,x,y,[(y+3)/(x-2), y]);
