...
matrix_for_basis_at_x0(f,x,y,divisor,basis,x0) :=
 block([ys, A, required_degrees], local(A),
    ...
    A[j,i] := coeff(puiseux(f,x,y,x0,ys[j],required_degrees[j],basis[i])[1][1], t, required_degrees[j]),
...
expansions_at_x0(f,x,y,divisor,basis,x0) :=
 block([ys, A, required_degrees], local(A),
    ...
    /* Now compute a matrix of the coefficients in the Puiseux expansion */
    A[j,i] := puiseux(f,x,y,x0,ys[j],5,basis[i]),
    ev(genmatrix(A,length(ys),length(ys)), puiseux_fracexp: false)
)$

basis_of_multiples_except_at_inf(f,x,y,divisor) :=
 block([basis,xs,x0,unstable],
    /* First, construct a basis for all rational functions */
    /* basis : [1,y], */
...
normal_matrix_at_inf(f,x,y,basis) := block([colord, A], local(A),
