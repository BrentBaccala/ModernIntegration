...
\mychapter{Abelian Integrals: The Algebraic Component}
...
matrix_for_basis_at_x0(f,x,y,divisor,basis,x0) :=
 block([ys, cycles, A, required_degrees, pairs, expansions], local(A),
    ...
    pairs : apply(append, map(lambda([i], makelist([ys[i],j], j, required_degrees[i], required_degrees[i] + cycles[i] - 1)),
    ...
    transpose(apply(matrix, map(lambda([basis_func],
               apply(append, map(lambda([pair],
                  append(map(lambda([exp], coeff(exp[1], t, pair[2])), puiseux(f,x,y,x0,pair[1], pair[2],basis_func)))),
               pairs))),
        basis)))
)$
...
expansions_at_x0(f,x,y,divisor,basis,x0) :=
 block([ys, A, required_degrees], local(A),
    ...
    /* Now compute a matrix of the coefficients in the Puiseux expansion */
    A[i,j] := puiseux(f,x,y,x0,ys[i],5,basis[j]),
...
basis_of_divisor_except_at_inf(f,x,y,divisor) :=
 block([basis,xs],
    ...
    for x0 in xs do block([target],
      ...
      target: lmax(cons(0, map(lambda([d], d[2]), sublist(divisor, lambda([d], d[1][1] = x0))))),
...
riemannroch2(f,x,y,divisor) := block([basis, colord, a],
...
\begin{maximablock}
basis : riemannroch(y^2 - x^2 - 1, x, y, [[[inf, inf], -1]]);
\end{maximablock}
...
there, because of the presence of the constant of integratio, so we
can always add a multiple of $1$ to our solution and get another
solution.

What about the two remaining basis elements?  We want to
...
\begin{maximablock}
puiseux(y^2 - x^2 - 1, x, y, inf, false, 0, 1);
puiseux(y^2 - x^2 - 1, x, y, inf, false, 0, x);
puiseux(y^2 - x^2 - 1, x, y, inf, false, 0, y);
\end{maximablock}

\begin{maximablock}
b : matrix([1], [-1]);
m : transpose(apply(matrix, map(lambda([b],
           map(lambda([e], coeff(e[1], t, -1)), puiseux(y^2 - x^2 - 1, x, y, inf, false, 0, b))),
    basis)));
pi : moore_penrose_pseudoinverse(m);
...
than the last one, so let's construct a Maxima function to extract the
principle parts of the basis functions and form them into a matrix:

\begin{maximablock}
principle_parts(f,x,y, func, divisor) :=
  flatten(
...
principle_parts_matrix(f,x,y, basis, divisor) :=
  transpose(apply(matrix, map(lambda([func], principle_parts(f,x,y,func,divisor)), basis)))
$
...
principle_parts2(f,x,y, func, divisor) :=
  flatten(
  map(lambda([d], map(lambda([e], map(lambda([order], coeff(e[1]/del(t), t, order-1)/order), makelist(order, order, d[2], -1))),
                      puiseux(f, x, y, d[1][1], d[1][2], 0, func))), divisor)
  )
$
...
This is an overspecified system -- a 6x8 matrix that may or may not
have a solution.  That's fine; since some integrals have no elementary
form, this doesn't represent a limitation in our theory, but a failure
to solve this matrix equation would prove the impossibility of
simplifying this integral into a Liouvillian form.
...
To proceed, we'll use the {\it Moore-Penrose pseudoinverse},
