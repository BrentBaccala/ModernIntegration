...
matrix_for_basis_at_x0(f,x,y,divisor,basis,x0) :=
 block([ys, cycles, A, required_degrees, pairs], local(A),
    ...
    /* A[i,j] := coeff(puiseux(f,x,y,x0,ys[i],required_degrees[i],basis[j])[1][1], t, required_degrees[i]), */
...
basis_of_multiples_except_at_inf(f,x,y,divisor) :=
 block([basis,xs,x0,unstable],
...
riemannroch(f,x,y,divisor) := block([basis, expansions, colord, nb, d],
  ...
  /* XXX this determinant has to be non-zero for the basis to be normal at inf */
