...
[Q2, B2]: divide(T2*R, V, x)$
h ::: A*num*y/(V*U) - (D(V)*Q2+D(B2))*y/V + Q2*D(y)$
\end{maximacode}

Compute: $$ \maxima[chebyshev]{'integrate(subst(y=sqrt(root), h), x);} $$

\begin{maximablock}[chebyshev]
f: y^2 - root;
solve(denom(h));
...
\begin{maximablock}[chebyshev]
mysimp(x) := signum(x)*sqrt(ratsimp(x*x))$
poles : map(mysimp, poles);
\end{maximablock}

Now we can see that all of our residues are $\pm\frac{5}{2}$, so we need only
a single logarithmic term.  Let's compute its residue:

\begin{maximablock}[chebyshev]
/* div : map(lambda([x], [[x[1], x[2]], signum(x[3])]), poles); */
div(p) := map(lambda([x], [[x[1], x[2]], p*signum(x[3])]), poles)$
\end{maximablock}

This calculation is computationally infesible for the present
Riemann-Roch basis space implementation.
