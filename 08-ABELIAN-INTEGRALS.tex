...
\begin{sageblock}[riemannroch2]
D2 = (x/y).divisor() + F(x).differential().divisor()
...
\begin{sageblock}[riemannroch2]
m2 = F.completion(D2.support()[0], prec=2)
m3 = F.completion(D2.support()[1], prec=2)
m2(x/y) * m2(x).derivative()
m3(x/y) * m2(x).derivative()
\end{sageblock}
...
# We'd like to do something like this (but this isn't supported):
#
# D = integrand.divisor_of_poles()
#
# and this, with absolute instead of relative precision:
#
# table([[pl, F.completion(pl)(integrand, prec=0)] \
#     for pl in D.support()])

D = (1/y).divisor() + F(x).differential().divisor()

table([[pl, F.completion(pl)(1/y, prec=2) * F.completion(pl)(x).derivative()] \
...
def logarithmic_divisor(differential, component):
    return add([- ZZ(differential.residue(pl) / component) * pl.divisor() for pl,m in differential.divisor().list() if m == -1])
\end{sageblock}
...
# this is divisor_of_poles(), which isn't defined for differentials
D = sum([-m * pl.divisor() for pl,m in integrand.divisor().list() if m < 0])

# D = integrand.divisor_of_poles()
# table([[pl, F.completion(pl)(integrand, prec=0)] \
#     for pl in D.support()])

#table([[pl, F.completion(pl)(y, prec=2) * F.completion(pl)(x).derivative() ] \
#    for pl,m in integrand.divisor().list() if m < 0])
table([[pl, F.completion(pl)(y, prec=2) * F.completion(pl)(x).derivative() ] \
    for pl in D.support()])
\end{sageblock}
...
\begin{sagecommon}
def differential_completion(map, differential):
    F = differential.parent().function_field()
    return map(differential/F.gen().differential()) * map(F.gen()).derivative()
def solution_vector(div, differential):
    F = differential.parent().function_field()
    coeffs = [(F.completion(p, prec=3), i) for p,m in div.list() for i in range(-m,0)]
    return matrix([[differential_completion(c[0], differential)[c[1]-1]/c[1] for c in coeffs]]).transpose()
\end{sagecommon}
...
def differential_divisor_of_poles(differential):
    return sum([-m * pl.divisor() for pl,m in differential.divisor().list() if m < 0])
\end{sagecommon}

\begin{sageblock}[riemannroch3]
D = differential_divisor_of_poles(integrand * x.differential())
table([[pl, differential_completion(F.completion(pl, prec=5), (integrand * x.differential()))] for pl in D.support()])
\end{sageblock}
...
D = differential_divisor_of_poles(integrand)
table([[pl, differential_completion(F.completion(pl, prec=10), integrand)] for pl in D.support()])
\end{sageblock}
...
pls = [pl for pl in F.places_above(Rinf)]
Dinf = add([pl.divisor() for pl in F.places_above(Rinf)])

# originaly this was "F.completion(pls[0], uvar=1/y)(1/x)", but we don't have uvar
F.completion(pls[0])(1/x)
F.completion(pls[1])(1/x)
\end{sageblock}

Just checking that we've got the same result as from the kash code:

\begin{sageblock}[curtis]
F.completion(pls[0])(1/x)(F.completion(pls[0])(1/y).reverse())
F.completion(pls[1])(1/x)(F.completion(pls[1])(1/y).reverse())
\end{sageblock}
...
(proof in Chapter \ref{chap:The Risch Theorem} when we construct the Jacobian variety).  Thus, to
...
R2.<x> = QQ[]
zpoly5 = R2(str(zpoly)).numerator().change_ring(GF(5))
ppoly5 = R2(str(ppoly)).numerator().change_ring(GF(5))
\end{sageblock}

Pay attention to how the ideals' polynomials behaves when reduced mod 5.
$\sage[chebyshevQ]{zpoly} \mod 5 = \sage[chebyshevQ]{zpoly5}$,
and
$\sage[chebyshevQ]{ppoly} \mod 5 = \sage[chebyshevQ]{ppoly5}$,
