...
%    D = v.divisor()
% but computing the divisor of a differential isn't supported.

% Also, we'd like to do this:
%    table([[p, F.completion(p, prec=0)(v)] for p,m in D.list() if m < 0])
% but neither is computing a series expansion (completion) of a differential supported,
% so I have to split it apart (like below) and take the derivative of the series.

% I had written a 'table1' function (it's still in 01-INTRO, at the moment) to line up
% series expansions in a table so that their powers all align together vertically,
% but it depends on a modification to LaurentSeries that isn't present in a stock Sage.

% This slide looks fine without 'table1'.
...
% principal_parts_matrix is from the Algebraic Curves chapter
% solution_vector is from the Abelian Integrals chapter
% they're in sagecommon, we only have them here to show their definitions on a slide
\begin{sageblock}[chebyshev]
