...
\begin{sagecommon}
def puiseux(F, f, pl, uvar):
    if isinstance(f, sage.rings.function_field.differential.FunctionFieldDifferential):
       f = f._f
       is_differential = True
    else:
       is_differential = False
    series = F.completion(pl, prec=4)
    uvar_series = series(uvar).reverse()
    f_series = series(f)(uvar_series)
    if is_differential:
       f_series *= series(f.parent()._gen_base_differential).derivative()(uvar_series)
    return f_series

def puiseux2(F, pl, uvar=None):
    def puiseux3(f):
       if isinstance(f, sage.rings.function_field.differential.FunctionFieldDifferential):
...
\begin{center}
\includegraphics[width=\textwidth]{07-ALGEBRAIC-CURVES-EXAMPLE1.pdf}
\end{center}
