...
def riemannrochKash(f, ZeroIdeal, PoleIdeal, multiple=1):
    ...
    # We passed in homogeneous ideals of the projective coordinate
    # ring, but Kash works in affine space, wants ideals of maximal
    # orders, requires the finite and infinite parts of the divisor to
    # be handled separately.  Also, ideals of the coordinate ring
    # specify Weil divisors, which don't map surjectively to Cartier
    # divisors (specified by ideals of maximal orders) at
    # singularities.

    # This is only working well enough for the examples in this
    # section.  We assume that there are no singularities at finite
    # points, so finite components can be converted directly into
    # ideals of the maximal finite order.  We also assume that there's
    # a single point at infinity, and if it appears in the coordinate
    # ring ideal, then it applies equally to all places at infinity.
    # Not really what we want, but consistent with how I wrote the
    # Maxima code (if a point is singular, assign the same degree to
    # all places over it).
    ...
    def ideal2divisor(Ideal):
       ...
       for q,p in Ideal.complete_primary_decomposition():
          if zz in p.gens():
             # infinite component
             infinite_prime_ideals = Oi.Decomposition()
             for i in range(1, infinite_prime_ideals.Length()+1):
                D = D + Integer(q.0.degree()) * infinite_prime_ideals[i].Divisor()
          else:
             # finite component
             D = D + O.Ideal(*[poly2kash(g, ybar) for g in q.gens()]).Divisor()
       return multiple * D

    # The system sum() doesn't work on Kash divisors.  It produces:
    # TypeError: unsupported operand type(s) for +: 'int' and 'KashElement'
    # (we don't actually use this function, though)
...
# Right now, only my patched Sage can compute intersections over QQbar

def intersection(List):
   return reduce(lambda i,j: i.intersection(j), List)

riemannrochKash(f, intersection(Zlist), I)

Zlist = [P(1,1), P(1,-1), P(-1,1), P(-1,-1), inf];
riemannrochKash(f, intersection(Zlist), I)
...
O.ideal(x-QQbar(-1).sqrt(), y-QQbar(2).sqrt())
F.places_infinite()

principal_parts_matrix()
\end{sageblock}
\end{comment}
