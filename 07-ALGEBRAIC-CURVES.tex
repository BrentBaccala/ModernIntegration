...
$K$ is the curve's field of constants.  The easy case is to let $K$ be the complex numbers $\mathbb{C}$.
...
A {\bf valuation ring} of a function field $F/K$ is a subring $O \in F$ such that
   %% We need this first statement in the definition, because otherwise we'd have two
   %% valuation rings for each place, with the elements in the ring and the elements not in the ring swapped.
   %% So we force our constants to be in valuation ring.  We also don't want to consider the
   %% entire function field as a valuation ring, so we require the second inclusion to be proper.
...
the valuation as $F_P := O/P$.  Given any element of $O$, its {\it residue} at the corresponding place is its
...
Intuitively, each distinct valuation ring corresponds to a place on the algebraic curve.
The residue of an element is its value at the place (obtained by substituting the place's
coordinates into the element's formula), and its value is its order (the lowest exponent
that appears in its Puiseux expansion).

% \cite{Swanson Huneke} proposition 6.3.4
% shows that a DVR is a principal ideal domain (and other equivalences).
% \cite{Swanson Huneke} Theorem 6.4.3 suggests that the Noetherian property
% (of the coordinate ring?) is what we need.
% The wikipedia page cited above shows that we need the Noetherian property, plus Krull dimension one.
...
\proof
\begin{enumerate}
\item Every cycle induces a unique valuation ring
...
{\bf Uniqueness}: We wish to show that for any two different cycles $A$ and $B$,
there is at least one function field element in $A$'s valuation ring that is not in $B$'s.
In other words, we're looking for an element that has a pole at $B$ but not at $A$.

Q: If both of the uniformizing variables have order one in each other's place,
then are all uniformizing variables of one also u.v.'s of the other?

A: Yes.  A uniformizing variable is any element of order one.  If a element of
order one is written out w.r.t. any uniformizing variable $v$, it will have
the form $c_1 v + \cdots$, and two expansions of this form can be composed
to get another expansion of this form.  Therefore, two places either have
compatible uniformizing variables, in which can any uniformizing variable
at the one place will be a uniformizing variable at the other, or they
are incompatible, in which case the two places have no uniformizing
variables in common.

The two different cycles $A$ and $B$ will have uniformizing variables $v_A$
and $v_B$.  If the cycles are not compatible,
$v_A$ is not also a uniformizing variable of $B$, then $v_A$'s expansion
in $v_B$ will not have order one.  If $v_A$ has negative order at $B$, then $v_A$ has
a pole at $B$ but not at $A$.  Likewise, if $v_B$ has negative order at $A$, then
$v_B$ has a pole at $A$ but not at $B$.
If $v_A$ has zero order at $B$, then $v_A^{-1}$ will also have zero order
at $B$ (and be in its valuation ring), but will have a pole at $A$.
Likewise, if $v_B$ has zero order at $A$, $v_B^{-1}$ will meet our requirements.
Finally if $v_A$ has positive order $r>1$ at $B$ and $v_B$ has positive order $s>1$ at $A$,
then $v_A/v_B$ has positive order at $B$ and negative order at $A$.

%% v_A = c_1 v_B^r + ... = c_1 (c_2 v_A^q + ...)^r + ... = c_1 c_2 v_A^(qr) + ...     v_A's expansion at B
%% v_B = c_2 v_A^q + ...                                                              v_B's expansion at A
%% they're not compatible because the expansions are only valid at the place they correspond to

If the uniformizing variables are compatible (they have order one at both places), then either $x$ or $y$
will have different expansions at the two places in terms of $v$, so subtract off
the common prefix $p$ (a polynomial in $v$) to get $(x-p)/v^r$ where $r$ is the order
of $x-p$.  $(x-p)/v^r - c_A$ will be zero on $A$ and non-zero on $B$.  Inverting it
will produce an element with a pole at $A$ and finite at $B$.  This element will
be in $B$'s valuation ring but not in $A$'s.
...
%% I don't think we need this.  The first two should be enough.
%%
...
There are an infinite number of valuation rings for any algebraic curve over an infinite field
of coefficients (like the complex numbers).  It is most convenient to separate them
...
% Reducing an element $f \in O$ modulo the HNF basis of $I$ gives a
% $k[x]$-vector that represents $f \bmod I$, i.e, when this
% $k[x]$-vector is multiplied by the HNF basis vector, we obtain an
% element of $O$ equivalent (mod $I$) to $f$.
...
{
\node {\bf Rational Functions}; & \node{\bf Algebraic Curve}; & \\
\node (Fx) {$C(x)$}; & \node (Fxy) {$C(x,y) \mod p(x,y)$}; & \node {Fraction Fields}; \\
                     & \node (I) {${\cal I}$}; & \node [align=center] {Maximal Finite Order /\\ Integral Basis}; \\
\node (Cx) {$C[x]$}; & \node (Cxy) {$C[x,y] \mod p(x,y)$}; & \node {Coordinate Rings}; \\
};
\draw [thick, ->] (Cx) -- (Fx);
\draw [thick, ->] (Fx) -- (Fxy);
\draw [thick, ->] (Cx) -- (Cxy);
...
def puiseux(F, pl, uvar=None, absprec=0):
    def puiseux2(f):
       ...
       if uvar:
          if isinstance(uvar, sage.rings.laurent_series_ring_element.LaurentSeries):
...
\ref{Stichtenoth} Definition 1.5.6 defines a Weil {\it differential}.
