...
puiseux2(f,x,y,x0,y0,deg,r,s,p) :=
 block([n:max(deg,s), xexpansion, yexpansion, res, allans, ans, eqns],
  if x0 # inf then
    xexpansion: x0 + t^r
  else
    xexpansion: (1/t)^r,
...
puiseux(f,x,y,x0,y0,deg,[ratfunc]) :=
 block([mp, pair, delta, g, r, s, p, yseries, rf, result],
   if y0 # inf then (
       ...
       */
      mp: lower_convex_points(monomial_powers(f,x,y,x0,0)),
      pair: lastn(mp,2)
   ),
   delta: pair[2]-pair[1],
   ...
   if length(ratfunc) > 0 then block([b: modulo(ratfunc[1], f, y)],
     ...
     for i: 0 thru hipow(b, y) do (
        /* deg-s is the highest t-degree required.  Divide by r to get x-degree required */
        rf: rf + subst(if x0 # inf then x=x0+t^r else x=1/t^r, taylor(ratsimp(coeff(b, y, i)), x, x0, ceiling((deg-s)/r)))*y^i
     )
   ) else rf: y,
   ...
   map(lambda([yseries],
      ...
      if puiseux_fracexp then
         ...
         [result, if x0 # inf then t^r=x-x0 else t^r=1/x]
   ), yserieslist)
 )$
