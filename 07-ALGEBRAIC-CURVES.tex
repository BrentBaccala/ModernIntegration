...
# XXX need to extend by i
y.divisor();
O = F.maximal_order()
pl = O.ideal(x-1, y).place()
xseries = F.completion(pl, prec=5)(x)
yseries = F.completion(pl, prec=4)(y)
\end{sageblock}

This answer differs from the one we computed by hand because the
choice of uniformizing variable ($s$) is not unique, and because the
computer made a different choice than did.  Our ``$t$'' variable was
roughly $\sqrt{x-1}$.  Even though square roots don't exist per se in
this field, we can take the square root of a power series that starts
with a square term.  Performing that operation on {\tt xseries}, we
get a series expansion for $\sqrt{x-1}$ (our $t$) in terms of the
computer's $s$.  Reversing the series gives us a series for $s$ in
terms of $t$, and substituting this series in for $s$ confirms
our previous calculation:

\begin{sageblock}[ch7]
t = (xseries-1)^(1/2)
S = LaurentSeriesRing(t.base_ring(), name='t')
s = S(t.reverse())
xseries(s)
yseries(s)
\end{sageblock}
...
# [(F.completion(pl, prec=4)(x), F.completion(pl, prec=4)(y)) for pl in Pinf]
[F.completion(pl, prec=4)(y)((1/F.completion(pl, prec=4)(x)).reverse()) for pl in Pinf]
\end{sageblock}
...
F.<y> = R.extension(y^2 + x^2 - 1)
D = (1/y).divisor()
[F.completion(p, prec=0)(1/y) for p,m in D.list() if m < 0]
\end{sageblock}
...
D = v.divisor()
[F.completion(p, prec=0)(v) for p,m in D.list() if m < 0]
\end{sageblock}
