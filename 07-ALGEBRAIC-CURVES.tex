...
# In puiseux2, we assume that any variable starting with 'd' is a differential
...
def puiseux2(origf, x0, y0, n, *args, **kwargs):
    ...
    if P.base_ring() == QQbar:
        ...
        p = puiseux2(newf, elem_dict[x0], elem_dict[y0], adjn)
...
\begin{sageblock}
analyze_multiple_points(y^4 - 4*y - x);
\end{sageblock}
...
\begin{sageblock}
analyze_multiple_points(y^3-3*y^2+x^6);
\end{sageblock}
...
% This one works, but takes several minutes to compute

\begin{sageblocksmall}
analyze_multiple_points(y^5 + (x^2-1)*y^4 - (4^4)/(5^5)*x^2*(x^2-1));
\end{sageblocksmall}
...
\begin{sageblock}
singular.lib('brillnoether.lib');

def riemannroch(CurveIdeal, ZeroIdeal, PoleIdeal):
   def simplify(x):
      if x in CurveIdeal.ring().base_ring():
         return CurveIdeal.ring().base_ring().one()
      else:
         return x / x.factor().unit()
   K = singular.RiemannRochBN(CurveIdeal, ZeroIdeal, PoleIdeal)
   denom = K[2].sage()
...
C = ideal(f.homogenize(z));
P1 = ideal(x,y);
I = ideal(R(1));

riemannroch(C, P1^4, I)
\end{sageblock}
...
riemannroch(C, Pinf, P1)
riemannroch(C, Pinf^2, P1)
riemannroch(C, Pinf^3, P1)
riemannroch(C, Pinf^4, P1)
riemannroch(C, Pinf, I)
riemannroch(C, Pinf^2, I)
riemannroch(C, Pinf^3, I)
\end{sageblock}
