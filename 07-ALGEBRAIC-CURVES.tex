...
the matrix by $(x,0,-1)$ to obtain $(x,0,1)$,
which, when dot-multiplied by $(1,y,y^2)$, yields $x+y^2$, not what we want.

Is this why we used reversed HNF?

On the other hand, if we order the basis $(y^2,y,1)$, then our matrix
for $y-1$ is:
...
Putting this matrix in HNF we get:
...
This {\it is} what we want: the $x-1$ is in the column corresponding to 1.
