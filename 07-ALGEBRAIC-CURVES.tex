...
puiseux2(f,x,y,x0,y0,deg,r,s,p) :=
 block([n:deg, /* ratwtlvl:deg,ratweights:[], */ xexpansion, yexpansion, res,ans:[],eqns],
  /* ratweight(x,1), */
  if x0 # inf then
    xexpansion: x0 + t^r
  else
  ...
  for i:0 thru n-s do block([s:solve(eqns[i+1],a[s+i])],
    if listp(s) and length(s) >= 1 then ans:endcons(s[1],ans),
    eqns: subst(ans,eqns)),
  subst(ans,yexpansion))$
...
puiseux(f,x,y,x0,y0,deg,[ratfunc]) :=
 block([mp, delta, g, r, s, yseries, rf, result, ratwtlvl: deg, ratweights:[]],
   if y0 # inf then (
   ...
   r: delta[1]/g,
   s: -delta[2]/g,
   /* if we're multiplying by a rational function in x, expand it */
   /* in a Taylor or Laurent series */
   if length(ratfunc) > 0 then block([b],
     /* rf: ratsimp(subst(x=x0+t^r, ratfunc[1]-b)), */
     /* rf: puiseux2(rf, t, b, 0, 0, deg, 1, -2, 0), display(rf) */
     rf: subst(if x0 # inf then x=x0+t^r else x=1/t, taylor(ratfunc[1], x, x0, deg))
   ) else rf: 1,
   /* expand y to the requested degree, adjusted by the degree of rf */
   if y0 # inf then
      yseries: puiseux2(f,x,y,x0,y0,deg+lopow(rf,t),r,s,mp[1][2]*r)
   else
      yseries: puiseux2(f,x,y,x0,0,deg+lopow(rf,t),r,s,0),
   /* ratsimp to collect like terms together, but we get a common denominator */
   result: ratsimp(rf*yseries),
...
\begin{maximacode}
plot3d ([sqrt((x**2-y**2+1)**2+(2*x*y)**2),
...
\begin{maximablock}
ev(puiseux(y^2 - x^2 - 1, x, y, inf, inf, 3, 1/(x^2+1)),
    puiseux_fracexp=false);
ev(puiseux(y^2 - x^2 - 1, x, y, inf, inf, 3, x/(x^2+1)),
    puiseux_fracexp=false);
\end{maximablock}

Now it appears that we have no pole, the expansion indicating simply
that $\lim_{x\to\infty}\frac{x}{\sqrt{x^2+1}} = 1$, but again
