...
puiseux3(f,x,y,x0,y0,deg,r,s,p,b) :=
 block([xsubst, tsubst, differential: 1, adjustment: 1, maxpow, rf:0, yserieslist, result],
   ...
   /* therefore, deg-s is the highest t-degree required in rf's expansion */
   ...
   /* Divide by r to get x-degree required. */

   maxpow: ceiling(maxpow/r),
   ...
   for i: 0 thru hipow(b, y) do (
      rf: rf + ratsimp(subst(xsubst, taylor(ratsimp(coeff(b, y, i)),
                                            x, x0, maxpow))
                       * adjustment)
               * y^i
   ),
   ...
   yserieslist: puiseux2(f,x,y,x0,y0,deg+lopow(rf,t),r,s,p),
...
\begin{maximablock}
puiseux(y^2 + x^2 - 1, x, y, 1, 0, 1, 1/y * del(x));
puiseux(y^2 + x^2 - 1, x, y, inf, inf, 12, x/y);
puiseux(y^2 + x^2 - 1, x, y, inf, inf, 10, x/y * del(x));
\end{maximablock}
...
\begin{maximablock}
puiseux(f, x, y, %i, sqrt(2), -1, integrand);
puiseux(f, x, y, cis(%pi/8), 0, -1, integrand);
