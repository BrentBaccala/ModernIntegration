...
Definition: An order $O$ of a ring $R$ (also called an $R$-order) is a subring of $R$ such that...
...
\begin{enumerate}
\item The coordinate ring is $R[x,y] \mod p(x,y)$
\item All points correspond to maximal ideals in the coordinate ring.
...
\item All places correspond to a maximal ideal in the ring of integral elements
...
Let $(x-x_0)$'s ramification index be $r$, so $x=x_0+t^r$ and let $(y-y_0)$'s valuation be $s$, which must be positive
if $(x_0,y_0)$ is a finite point on the curve, so $y=y_0 + y_s t^s + \cdots$
.  Consider $\frac{(y-y_0)^r}{(x-x_0)^s}$, an algebraic
function with valuation zero.  $r$ will be less than or equal to the degree of the defining
polynomial, but if it is equal, then the numerator could simplify, right?  Construct
$\frac{(y-y_0)^r}{(x-x_0)^s} - v_0$ where $v_0$ is the value of the function, and we've
got an algebraic function, zero at the place, whose only poles are at $x_0$.
What about other points over $x_0$? (it has poles)

\item Two different places (at the same point) correspond to two distinct maximal ideals.
\end{enumerate}

or
\begin{enumerate}

\item A place corresponds to a valuation ring

\item A valuation ring of a function field $F/K$ is a ring $O \in F$ such that
   $K \in O \in F$ (both proper inclusions)
   for every $z \in F$, either $z \in O$ or $z^{-1} \in O$

$z \in O$ if $z$ is finite (could be zero) at the place.

\item Need to show that every cycle induces a place

Pretty easy - use the Puiseux expansion of each function to compute its valuation.
...
\item Need to show that every place induces a cycle
...
\item Need to show that different cycles yield different places
...
\item Definition: A place $O$ is finite if $x \in O$.

\item The maximal finite order is the intersection of all finite places.

Show that it's an order.

Show that it's identical to the integral closure of the coordinate ring.

\item The infinite order is the intersection of all non-finite places.
