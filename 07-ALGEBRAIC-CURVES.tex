...
\begin{comment}
  ...
  if y0 # inf then
    ...
    yexpansion: sum((1/t)^i*a[i],i,s,n),
\end{comment}
...
puiseux(f,x,y,x0,y0,deg,[ratfunc]) :=
 block([mp, delta, g, r, s, yseries, rf, result, ratwtlvl: deg, ratweights:[]],
   ...
   if length(ratfunc) > 0 then block([b],
     ...
     rf: subst(x=x0+t^r, taylor(ratfunc[1], x, x0, deg))
   ) else rf: 1,
   /* expand y to the requested degree, adjusted by the degree of rf */
   yseries: puiseux2(f,x,y,x0,y0,deg+lopow(rf,t),r,s,if y0 # inf then mp[1][2]*r else 0),
