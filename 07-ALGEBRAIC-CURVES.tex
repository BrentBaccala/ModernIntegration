...
respect to a mapping that in this case is a restriction map from the
curve to one of its coordinate variables.
...
Is the curve holomorphic at a ramification point?  Yes,
according to Riemann's theorem on removable singularities.
Wikipedia's statement of this theorem reads as follows:
...
The function $y$ is bounded (3), which implies (1), that it can
be holomorphically extended to $a$.

In short, we already know that to obtain a Riemann surface,
...
the behavior of meromorphic functions at those additional
...
What about functions with poles?
...
F.<y> = R.extension(y^2 + x^2 - 1)
D = (1/y).divisor()
table([[p, F.completion(p, prec=0)(1/y)] \
...
\begin{sageblock}[lemniscate]
Fs = O.ideal(x).factor()
F.completion(Fs[0][0].place(), uvar=x, prec=7)(y)
F.completion(Fs[1][0].place(), uvar=x, prec=7)(y)
F.completion(Fs[2][0].place(), uvar=x, prec=7)(y)
F.completion(Fs[3][0].place(), uvar=x, prec=7)(y)
\end{sageblock}
