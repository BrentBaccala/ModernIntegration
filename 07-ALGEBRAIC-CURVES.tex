...
def puiseux_cycle(f, x0, y0, n, hne):
    ...
    try:
        ...
        #p = eds.apply_map(f.parent().base_ring()) * matrix([t^i for i in range(1, eds.ncols()+1)]).transpose()
        p = eds.apply_map(ringmap) * matrix([t^i for i in range(1, eds.ncols()+1)]).transpose()
        #return (eds, matrix([t^i for i in range(1, eds.ncols()+1)]).transpose())
...
def apply_map_to_laurent_series(morphism, series):
    ...
    result = R(map(morphism, list(series)))
    result.add_bigoh(series.prec())
    return result
...
def apply_map_to_laurent_series(morphism, series):
    ...
    result = R(map(lambda x: x.radical_expression(), map(morphism, list(series))))
    result.add_bigoh(series.prec())
    return result
...
def puiseux(origf, x0, y0, n):
    ...
    if origf.parent().base_ring() == QQbar:
        ...
        #return (morphism, p)
        try:
            return [[apply_map_to_laurent_series(morphism, cycle[0]), apply_map_to_laurent_series(morphism, cycle[0])] for cycle in p]
        except TypeError:
            return (morphism, p)
