...
distribute_denominator_limited(frac, var, lim) := block(
  [result: 0, n: num(frac), d: denom(frac)],
...
puiseux(f,x,y,x0,y0,deg,[ratfunc]) :=
 block([result: [], rf],
   ...
   if y0 = false then block([ys, f0],
      ...
      ys : unique(radcan(rectform(solve(f0)))),
...
\begin{maximablock}
analyze_multiple_points(y^3-3*y^2+x^6, x, y)$
puiseux(y^3-3*y^2+x^6, x, y, 0, 0, 5);
puiseux(y^3-3*y^2+x^6, x, y, 0, 3, 5);
\end{maximablock}
