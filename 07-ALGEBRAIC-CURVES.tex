...
def puiseux(f, x0, y0, n):
    ...
    if ed[3] == 1:
        ...
        (x0,y0) = (y0,x0)
    eds = ed[1].sage()
    p = eds * matrix([(x-x0)^i for i in range(1, eds.ncols()+1)]).transpose()
    #return (y, p[0,0])
    #PSring = PowerSeriesRing(f.parent(), f.parent().gens())
    PSring = Frac(f.parent().base_ring()[[x]])
    PSelem = y0 + PSring(p[0,0]).add_bigoh(n+1)
    return (SR.var('y') == SR(PSelem))
    #return (y, PSelem)
