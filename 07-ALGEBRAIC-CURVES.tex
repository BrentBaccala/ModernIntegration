...
newton_polygon(x, y, list) := block([result, first, slopes, lowest_slope],
   /* step 1 - discard all points directly above other points */
   /* step 2 - sort the list by increasing x values */
   /* first: sort(list, lambda([u,v], u[1]<v[1] or u[2]<v[2]))[1], */
   list: sort(list, lambda([u,v], u[1]<v[1])),
   while length(list) > 0 do (
      first: pop(list),
      result: [first],
      slopes: map(lamba([u], ((first - u)[2] / (first-u)[1])), list),
      lowest_slope: sort(slopes)[1],
      while (slopes[1] > lowest_slope) do (pop(slopes), pop(list)),
      while (slopes[1] = lowest_slope) do (pop(slopes), result:endcons(pop(list), result))
   ),
   result
)$
...
/* list is a list of pairs, like xy */
newton_polygon(list) := block([result, nxt, slopes, lowest_slope],
   /* step 1 - discard all points directly above other points */
   /* step 2 - sort the list by increasing x values */
   list: sort(list, lambda([u,v], u[1] < v[1])),
   result: [pop(list)],
   while length(list) > 0 do (
      ...
      lowest_slope: sort(slopes)[1],
      if lowest_slope >= 0 then return(result),
...
lower_convex_hull(list, x) := block(
...
plot2d([[discrete, xy], 'lower_convex_hull(newton_polygon(xy), l)], [l,0,3], [y,0,3],
...
puiseux(y^2 - (t^2+%i)^2 - 1, t, y, 0, 0, 5);
subst(t=sqrt(x+%i), %);
\end{maximablock}
