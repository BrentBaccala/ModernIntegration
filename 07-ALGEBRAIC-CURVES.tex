...
def puiseux_cycle(f, x0, y0, n, hne):
    ...
    ed = singular.extdevelop(hne, adjn-1)

    # We get a flag indicating which variable was used as a
    # uniformizing parameter.  Swap variables if y was the
    # uniformizing parameter (we'll swap back later).
...
def puiseux2(origf, x0, y0, n, *args, **kwargs):
    ...
    if len(args) > 0:
        assert(len(args) == 1)
        (x,y) = P.gens()
        # put the function in K(x)[y]/(p)
        R = P.base_ring()[x]
        F = Frac(R)[y]
        Q = QuotientRing(F, F(origf))
        fn = Q(numerator(args[0])) / Q(denominator(args[0]))
        ...
        for e in range(Q.degree()):
            if fn[e] != 0:
                ...
                prec = max(prec, n - (cv-1)*xv - e*yv)
                if e > 0:
                    prec = max(prec, n - cv*xv - (e-1)*yv)

        p = puiseux(origf, x0, y0, prec)
        ans = []
        for cycle in p:
           # XXX only add bigoh if precision is finite
           ans.append(args[0].subs({x: cycle[0], y: cycle[1]}).add_bigoh(n))
        return ans
