...
Due to the complexity of this integral, I'll use Maxima for the entire calculation.
...
Now let's tell Maxima how our variables differentiate:

\begin{maximablock}
gradef(theta, x, 1/(x+1))$
gradef(psi, x, 2*x*psi)$
\end{maximablock}
...
\begin{maximablock}
pf: partfrac(N/denom(lintegrand), theta);
\end{maximablock}
...
\begin{maximablock}
den(pl,v) := partition(denom(pl),v)[2]$
denoms: map(lambda([u], den(u,theta)),
            partlist(pf));
\end{maximablock}
...
vtick: D(v);
N/denom(lintegrand);
r: gcdex(v,vtick,theta) * N/(x+1)$
S::: r[1] - divide(r[1],vtick,theta)[1]*vtick;
T::: r[2] + divide(r[1],vtick,theta)[1]*v;
\end{maximablock}
...
\begin{maximacode}
/* a and N are used again in example 6.8, so don't kill them */
kill(A, pf, n, R, b, B, Q, C)$
kill(v, vtick, r, S, T, A, B)$
\end{maximacode}
