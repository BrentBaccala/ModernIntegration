...
This has the form ?? with $n_1(\theta) = \theta$ with $b_{1,1}(\theta) = \frac{1}{x}$.
...
This has the form ?? with $n=1$ and $a_1 = 1$, so the integral can have degree two, and $A_2$ is a constant.

$$A_1' = a_1 - 2 A_2 \frac{k'}{k} = 1 - 2 A_2 \frac{1}{x}$$

$$A_1 = x - 2 A_2 \theta$$

Since $A_1 \in {\bf C}(x)$, it can not involve $\theta$, so $A_2$ must be zero.  So $A_1$ is $x$ and

$$A_0' = a_0 - A_1 \frac{k'}{k} = - x \frac{1}{x} = -1$$
...
This is in the form ?? with $n=1$ and $a_1=\frac{1}{2}i$, so
