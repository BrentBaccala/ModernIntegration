...
a differential field $K$, and let $x=\sum a_i \theta^i$ be a
polynomial in $K[\theta]$.  Then:

\begin{enumerate}
\item If $x$'s leading coefficient is constant ($a_n' = 0$), then ${\rm Deg}_\theta\, x' = {\rm Deg}_\theta\, x - 1$
\item If $x$'s leading coefficient is not constant ($a_n' \ne 0$), then ${\rm Deg}_\theta\, x' = {\rm Deg}_\theta\, x$
\item If $x$ is monic and irreducible, then $x' \nmid x$
\end{enumerate}

\proof

The first two statements follow easily from considering $x'$:

$$x'=\sum_{i=0}^n (a_i' \theta^i + i a_i \theta' \theta^{i-1}) = \sum_{i=0}^n
\left(a_i' + \left(i+1\right)a_{i+1} \theta'\right) \theta^i$$

Note that since $K(\theta)$ is a logarithmic extension, $\theta'
\in K$, so for all $i$ the entire expression $(a_i' + (i+1)a_{i+1} \theta')$
is in $K$.  In particular, since $a_{n+1}$ is zero, the $n^{\rm th}$
coefficient of $x'$ is just $a_n'$ and the $\theta$-degree of $x'$
will be $n$ if $a_n'$ is non-zero.  On the other hand, if $a_n'$ is
zero, then the $n-1^{\rm th}$ coefficient of $x'$ is $(a_{n-1}' + n
a_n \theta')$ which would be zero only if $\theta' =
-\frac{a_{n-1}'}{n a_n} = (-\frac{a_{n-1}}{n a_n})'$ (by Theorem
\ref{basic difalg} since $a_n$ is constant), which implies
an algebraic relationship between $\theta$ and $-\frac{a_{n-1}}{n
a_n}$ (specifically, they differ only by a constant), contradicting
the transcendence of $E$ over $K$.

Finally, if $x$ is monic and irreducible, then ${\rm Deg}_\theta\, x' =
{\rm Deg}_\theta\, x - 1$, and no lower degree polynomial can divide
...
let $x=\sum a_i \theta^i$ be a polynomial in $K[\theta]$.  Then:

\begin{enumerate}
\item ${\rm Deg}_\theta\, x' = {\rm Deg}_\theta\, x$
\item $x' \mid x$ if and only if $x$ is monomial.
\end{enumerate}
...
$$x' = \sum_{i=0}^n (a_i' \theta^i + i a_i \theta' \theta^{i-1})$$

This time, however, $\theta' = k'\theta$, so

$$x' = \sum_{i=0}^n (a_i' + i a_i k') \theta^i$$

Assume that one of these coefficients, say $(a_i' + i a_i k')$, was
zero but $a_i$ was non-zero.  Then $D(a_i \theta^i) = (a_i' + i a_i
k') = 0$, so $a_i \theta^i$ would be a constant, which must be in $K$,
contradicting the transcendence of $E$.  Therefore, none of these
coefficients can be zero, establishing the first claim.

To establish the second claim, assume first that $x' \mid x$.  Since
$x'$ has the same degree as $x$ (by the first claim), it can only
divide $x$ if it has the form $mx$, where $m \in K$.  Equating
coefficients of $\theta$ in the above sums leads us to conclude that

$$m = (\frac{a_i'}{a_i} + i k')$$

If $x$ was not monomial, then all of its coefficients must
yield the same value for $m$, i.e,

$$m = (\frac{a_i'}{a_i} + i k') = (\frac{a_j'}{a_j} + j k')$$

$$a_i' a_j - a_i a_j' + (i - j) k' a_i a_j = 0$$

$$\frac{a_i' a_j - a_i a_j'}{a_j^2} + (i - j) k' \frac{a_i}{a_j} = 0$$
$$\left(\frac{a_i}{a_j}\right)' + (i - j) k' \frac{a_i}{a_j} = 0$$

Then $D(\frac{a_i}{a_j} \theta^{j-i}) = \left(\frac{a_i}{a_j}\right)'
+ (i - j) \frac{a_i}{a_j} k' = 0$, again contradicting the
transcendence of $E$ over $K$.  So $x$ most be monomial.

Conversely, assume $x$ is monomial, say $a\theta^n$.  Then $x' = (a' +
n a k') \theta^n = \frac{a' + n a k'}{a} x$ and $x' \mid x$.

\endtheorem

\example Let $x = x e^x$.  Then
...
the ring $K[\theta]$.  Both $x$ and $x'$ are in $K[\theta]$; note
that $\frac{x+1}{x} \in K$, so $x' \mid x$ in this ring.
