...
a differential field $K$, let $p=\sum p_i \theta^i$ be a polynomial in
$K[\theta]$, and let $r = a/b$ be a rational function in $K(\theta)$
($a, b \in K[\theta]$).  Then:
...
\item $r' \in K$ only if $r$ has the form $c\theta + k$, where $c$ is a constant
\end{enumerate}
...
p_n}$ (specifically, they differ only by a constant), contradicting
