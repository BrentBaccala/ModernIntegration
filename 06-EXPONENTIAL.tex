 ...
 * denominator, [factor, S-power, T-power]
 */
...
normal_risch_equation(S, T, var) := block(
   ...
   sf: map(lambda([u], [base(u), power(u)]),
           factors(denom(ratsimp(S)), var)),
   tf: map(lambda([u], [base(u), power(u)]),
           factors(denom(ratsimp(T)), var)),
   map(lambda([u], [u, assoc(u, sf, 0), assoc(u, tf, 0)]), n)
)$
