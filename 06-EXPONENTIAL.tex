...
AA + D(psi)/psi*A = lintegrand/psi;
(theta+x)^2*%;
ratexpand(%);
/* AA + ratsimp(D(psi)/psi)*A = lintegrand/psi; */

((1+1/x^2)*theta^2 + (2*x - 1 + 2/x)*theta + x^2)/(theta+x)^2;
subst(theta=exp(x),%);
integrate(%,x);

expand(ratsimp(D((x^2-1)/x+1/(theta+x)) * (theta+x)^2));
\end{maximablock}
