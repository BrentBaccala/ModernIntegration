...
I'll begin by presenting the basic structure theorem for exponential
...
\begin{multline*}
F' = \bar{A}_0 + \sum D_i k' \deg_\theta n_i(\theta) + \sum_{i=1}^n (A_i' + i A_i k' )\theta^i
  + \sum_{j=1}^{l} \frac{B_{j}' - j k' B_{j}}{\theta^j} \\
...
$$A_0' + \sum D_i k' \deg_\theta n_i(\theta) = a_0$$
...
$$ D_1 \left[ n_1(\theta)' - j k' n_1(\theta) \right] = c_{1,1}
\qquad D_2 \left[ n_2(\theta)' - j k' n_2(\theta) \right] = c_{2,1}$$
...
$$-i \int \frac{\theta - \theta^{-1}}{\theta + \theta^{-1}} \,dx
  = (1-2i)x + \ln (\theta+i) + \ln(\theta-i)$$
$$  = (1-2i)x + \ln \left[ (\theta+i)(\theta-i) \right]$$
$$  = (1-2i)x + \ln \left[ \theta^2 + 1 \right]$$
...
\ref{exponential structure theorem}
