...
Having done so, we can replace $r$ with $p/d$:

$$A \frac{p'd-pd'}{d^2} + B \frac{p}{d} = C$$

$$A (p'd-pd') + B pd = C d^2$$

We still might have lingering denominators in $B$ and $C$,
which can be cleared:

$$A D (p'd-pd') + B D pd = C D d^2$$

$$(A D d) p' + (B D d - A D d')p = C D d^2$$
 ...
 *
 * If expr isn't a fraction, return an empty list
 * otherwise, return a list of the fractions
 * in a partial fractions expansion.
 */
...
partfrac1(expr,v) := block(
   ...
   expr1 : ratsimp(expr),
   if atom(expr1) then []
   elseif op(expr1) = "/" then partlist1(partfrac(expr1,v))
   else []
)$

/* Return a list of lists, one for each factor in S or T's
 * denominator:
...
normal_risch_factors(S, T, var) := block(
   [n, sf, tf],
   n: map(base,
          factors(denom(ratsimp(S)) * denom(ratsimp(T)), var)),
   sf: map(lambda([u], [base(denom(u)), [num(u), power(denom(u))]]),
           partfrac1(S, var)),
   tf: map(lambda([u], [base(denom(u)), [num(u), power(denom(u))]]),
           partfrac1(T, var)),
   sf: sort(sf, lambda([u,v], u[2][2] > v[2][2])),
   tf: sort(tf, lambda([u,v], u[2][2] > v[2][2])),
   map(lambda([u], [u, assoc(u, sf, [0,0]), assoc(u, tf, [0,0])]), n)
)$
...
normal_risch_denominator1(L) := block(
   ...
   elseif k = 1 then factor^block(
      ...
      if integerp(j) then max(l-1, j, 0) else max(l-1, 0)
   )
...
/* Given a normal Risch equation r' + Sr = T * and the denominator of r,
 * return the polynomial * equation satisfied by the numerator of r:
 *    A n' + B n = C
 * in the form [A,B,C]
 */
...
/* Given a normal polynomial Risch equation A n' + B n = C,
 * return the maximum degree of n, or -1 (no solution).
 */

normal_risch_degree(A, B, C, var) := block(
   [k, p],

   k : hipow(C, var) - max(hipow(A, var) - 1, hipow(B,var)),

   /* Can we have a constant solution?  Should just return it. */
...
normal_risch_equation(S, T, var) := block(
   ...
   elseif normal_risch_degree(A,B,C,var) = -1 then false
   else product(c[i] * var^i, i, 0, normal_risch_degree(A,B,C,var))/D
)$

normal_risch_equation(%i, 1/(2*%i), x);

normal_risch_equation(-2*x, 1, x);

normal_risch_equation(%i, 1/x, x);
