...
% $$A T F^{a+t} (N' F D - N(m D + F D')) - S N F^{s+1} D = C T D^2 F^{c+t+m+1}$$
...
\begin{comment}
% These equations assume that we've got F's in all three polynomials
$$A F^{a} r' - B F^b r = C F^{c}$$

% $$A F^{a} \frac{N' F^m D - N(m F^{m-1} F' D + F^m D')}{F^{2m} D^2} - B F^b \frac{N}{F^m D} = C F^{c}$$

%$$A F^{a} \frac{N' F D - N(m F' D + F D')}{F^{m+1} D^2} - B F^b \frac{N}{F^m D} = C F^{c}$$
...
\begin{comment}
% These equations assume that a might be something other than 1
$$A F^{a} r' - B r = A F^{a}$$
...
\begin{comment}
% Assuming that F has unit multiplicity
$$A r' - \frac{B}{F} r = C$$
...
%$$ - (m A F' - B ) N D  = A F (D^2 F^{m} - N' D - N D')$$
\end{comment}
