...
DD(f) := subst('diff(theta,x) = theta,
              diff(f,x))$
D(f) := subst(['diff(psi,x) = DD((x^2-1)/x+1/(theta+x))*psi,
               'diff(theta,x) = theta],
              diff(f,x))$
\end{comment}
...
integrand:
   (exp(x) - x^2 + 2*x)/((exp(x) + x)^2 * x^2)
   * exp((x^2-1)/x+1/(exp(x)+x));

gradef(theta, x, theta);
gradef(psi, x, D((x^2-1)/x+1/(theta+x))*psi);

lintegrand:
   (theta - x^2 + 2*x)/((theta + x)^2 * x^2) * psi;
\end{maximablock}

The integrand has the form $a_1 \psi$, so we can
apply Theorem \ref{exponential integration theorem}.
...
eq*x^2*(theta+x)^2;
facsum(%,rr,r);
[A,B,C] : [part(%,1,1)/rr, part(%,1,2)/r, part(%,2)];
\end{maximablock}
