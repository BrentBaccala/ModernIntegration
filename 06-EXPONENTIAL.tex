...
\begin{equation}
\label{eq: normal polynomial Risch}
...
% Labeling text with equation numbers
% https://tex.stackexchange.com/questions/114113
\makeatletter
\newcommand{\eqnum}[1]{\hfill\textup{\tagform@{\theequation #1}}}
\refstepcounter{equation}
\label{normal Risch cases}
\begin{enumerate}

\item $k=0$ and $j = l-1$. \eqnum{a}

\item $k=1$ and either $j=l-1$ or $j = \frac{b(\theta)}{n'(\theta)} \mod n(\theta)$. \eqnum{b}

\item $k>1$ and $j=l-k$. \eqnum{c}
...
\example
Integrate $\int \log(x) e^{-x^2} dx$

It's noted in the Sage documentation that Sage can't simplify this integral,
though both Maple and Mathematica can ({\tt calculus/tests.html}).
\endexample
...
This is already in the form required by equation \ref{eq: general polynomial Risch}:
...
(the multiplicity in $T$'s denominator).  Case 3 in \ref{normal Risch cases} tells us
that $j=l-k=0$, so neither of these (normal) factors can appear
in the denominator of $r$, and $r$'s denominator can
only involve special factors.
