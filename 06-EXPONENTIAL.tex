...
normal_risch_degree(S, T, var, D1) := block(
   [D2, A, B, C, j, k],
   S : ratsimp(S*D1),
   T : ratsimp(T*D1),
   D2 : denom(S)*denom(T),
...
/*
   if integerp(C/B) then k : max(k,0),

   if hipow(A, var) = hipow(B, var) + 1
   then block([p],
      p : - lc(B, var) / lc(A, var),
      if integerp(p) then max(k, p) else k)
   else k
*/
   [A, B, C]
)$
...
normal_risch_equation(S, T, var) := block(
   ...
   [D, normal_risch_degree(S, T, var, D)]
)$
...
normal_risch_denominator(-2*x, 1, x);

normal_risch_equation(%i, 1/x, x);

normal_risch_denominator(%i, 1/x, x);
