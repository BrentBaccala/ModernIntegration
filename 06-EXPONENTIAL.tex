...
\begin{maximablock}
depends(A,x)$
eq: D(A[1]) + D(psi)/psi*A[1] = lintegrand/psi;
\end{maximablock}
...
eq*x^2*(theta+x)^2;
facsum(%,A[1]);
[AA,B,C] : [part(%,1,1)/D(A[1]), part(%,1,2)/A[1], part(%,2)];
\end{maximablock}
...
lc(expand(AA),theta);
lc(B,theta);
integrate(lc(B,theta) / lc(expand(AA),theta), x);
\end{maximablock}
...
\begin{maximablock}
rrr: A[1,-1] * theta^-1 + A[1,0] + A[1,1] * theta;
D(rrr);
declare(theta,mainvar);
subst(A[1]=rrr, eq*x^2*(theta+x)^2*theta);
ev(%, diff);
part(%,1) - part(%,2);
/* facsum(%,r[-1],D(r[-1]),r[0],D(r[0]),r[1],D(r[1])); */
facsum(%,theta);
lc(expand(%), theta);

subst([A[1]=rrr, A[1,1]=0], eq*x^2*(theta+x)^2*theta);
ev(%, diff);
part(%,1) - part(%,2);
ratsimp(%);
lc(expand(%), theta);

subst([A[1]=rrr, A[1,1]=0, A[1,0]=0], eq*x^2*(theta+x)^2*theta);
ev(%, diff);
part(%,1) - part(%,2);
ratsimp(%);
lc(expand(%), theta);

subst([A[1]=rrr, A[1,1]=0, A[1,0]=0, A[1,-1]=1], eq);
ev(%, diff);
ratsimp(%);
\end{maximablock}
